# Deep Dive Completeness Checklist

Grade a deep dive here before calling it finished. Three gates: **Gate 0** (the five quality
layers — research vs deck), **Gate 1** (the must-have modules), **Gate 2** (craft).

Keep the layers *here*, on one page — don't sprinkle them through the write-up. During
writing it should feel like a **research flow**; here is where it gets graded like a checklist.

---

## GATE 0 — The Five Mandatory Layers (research vs deck)
- [ ] **A — Facts:** every material claim = **number + date + source**. No undated "is rising."
      **Data-density test: every CORE module carries ≥1 quantified, dated figure** (market size,
      CAGR, margin, mix %, ROCE…). A module with no numbers is a teaching slide, not research.
- [ ] **B — Economics:** profit pools + **who captures them** + **margin-migration direction**;
      companies judged on **returns on capital**.
- [ ] **C — Cycles:** **cycle position + a Cycle Dashboard of leading indicators**, with the
      **demand cycle separated from the stock cycle** where they diverge.
- [ ] **D — Failure Modes:** a **Graveyard** (who died + why + base rate) AND a **Live Risk
      Dashboard** (today's thesis-killers + tripwires).
- [ ] **E — Scorecards:** a **weighted, 1–5, rankable** rubric + a **Capital Allocation score**,
      with **≥1 company scored end-to-end**.

**Any layer unchecked → it's "educational," not "investment research." Fix before publishing.**

---

## GATE 1 — Must-have modules (the minimum bar)

**[CORE] — the 15–20 page narrative (the argument):**
- [ ] **Decision Summary / IC Page** — the read-first one-pager (industry call, best/worst
      segment, best company, biggest risk, variant view, what-changes-my-mind, monitoring metrics)
- [ ] **Research Question** — central uncertainty + prove/disprove + **time horizon declared**
      (trade / investment / structural)
- [ ] **Archetype tagged** (Commodity / Technology / Regulated / Infrastructure / **Network-Platform** / Hybrid)
      **+ archetype's mandatory deliverables present** (see `ARCHETYPES.md`)
- [ ] **Why now** — 3+ dated reasons
- [ ] **Plain-English definition** (5-year-old test)
- [ ] **How it's made** — process/pipeline + cost stack
- [ ] **Segments** broken out, each with a **tell metric**
- [ ] **Value-chain margin map** — pools + who captures them + migration direction
- [ ] **Industry Metrics Cheat Sheet** — the 5–10 insider ratios defined + what "good" looks like
- [ ] **Red Flag Checklist** — the industry-specific kill-criteria (inverse of the scorecard)
- [ ] **Industry structure** — concentration + barriers to entry
- [ ] **Cycle position** + **Second-Order Effects** chain (driver → outcome → consequence)
- [ ] **Management & Capital Allocation** with a score
- [ ] **Evaluation scorecard** (weighted) defined *before* company analysis
- [ ] **Top 2–3 company deep dives** with Balance Sheet + P&L + **ROCE/ROE**
- [ ] **Anchor company goes DEEP** — capital-allocation timeline (dated decisions) + 10-yr ROCE
      trend + management mistakes + an explicit **bear case** (no shallow company analysis)
- [ ] **Consensus vs Variant View** + a **Falsification Test** (what would prove me wrong?)
- [ ] **Valuation** (sector-appropriate + margin-of-safety) + **Investment Fit** (priced to cycle)
- [ ] **Conclusion** that answers the Research Question at the declared horizon (your own view)

**[APPENDIX] — the evidence room (optional depth, not in the main narrative):**
- [ ] **Cycle Dashboard** — leading indicators (current / 5Y avg / direction) → *where are we?*
      **— filled with actual values, not left as empty headers**
- [ ] **Industry KPI Dashboard** — operational scoreboard → *who is winning?*
      **— filled with a real cross-company comparison (≥3 players), not blank**
- [ ] **Full company scoring table** (≥1 company scored end-to-end, incl. Capital Allocation)
- [ ] **Acquisition / capital-allocation track record** — deal-by-deal, value created vs destroyed
- [ ] **Historical Graveyard** (how value dies here + base rate)
- [ ] **Live Risk Dashboard** (thesis-killers + tripwires)
- [ ] **Data sources / monitoring cadence** + a **Source Quality Table** (claims weighted by tier)
- [ ] **Research Log** — dated observations + impact on thesis (makes it a living document)

## GATE 2 — Craft (good → great)
- [ ] **Facts > opinions** — claims sourced, not vibes
- [ ] **Altitude shifts** — macro (why now) ↔ mid (structure/value chain) ↔ ground (companies)
- [ ] **Comparable company layouts** — same template for every company
- [ ] **Moat analysis per company** — Scale/Brand/Regulatory/Distribution/Pipeline/Switching rated
      (not all growth is equally durable)
- [ ] **Compliance scorecard** present when the archetype is Regulated (per-company, feeds the rubric)
- [ ] **Self-researchable** — a reader could update it from the sources listed
- [ ] **Visual where it counts** — value chain, timeline, Porter's, both dashboards as tables/diagrams
- [ ] **Archetype weighting respected** — ~60% effort on the over-weighted modules, full coverage elsewhere
- [ ] **Contrarian edge** — the variant view is genuinely non-consensus and evidenced, not a strawman
- [ ] **Readability governor honoured** — CORE narrative stays ~15–20 pages; heavy machinery is
      in the APPENDIX. If a reader can't reach the insight, the report failed.
- [ ] **Second-order thinking** — at least one driver→outcome→consequence chain chased 2–3 steps
- [ ] **Evidence → Interpretation → Conclusion** kept distinct on every major call (no
      fact/opinion blending)
- [ ] **Confidence tagged** [H/M/L] on the key conclusions — no false precision
- [ ] **Effort allocated by archetype** — hours actually went where ARCHETYPES says they should

---

## Generic data-source starter list
Adapt per industry; default kit observed across the decks.

**Primary / company-level:** annual reports, investor decks, earnings transcripts,
exchange filings & corporate announcements (BSE/NSE; SEC for US), capacity/order-book disclosures.

**Industry / macro:** sector regulator & ministry sites, policy docs (PLI, duties, NITI Aayog),
industry associations/councils, IEA, BloombergNEF, McKinsey/BCG/Bain, Goldman/BlackRock/Nomura,
specialist creators (YouTube/X/Substack), trade press (Bloomberg, CNBC, Quartz).

**Monitoring cadence:** define the 3–5 **tell metrics** for *this* industry (the Cycle Dashboard),
how often to refresh them, and from where. That dashboard is your living version of the deep dive.
